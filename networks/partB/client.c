...
int main()
{
    ...
    if (sendto(sockfd, &total_chunks, sizeof(total_chunks), 0, (struct sockaddr *)&server_addr, addr_len) == -1)
    {
    ...
    {
        chunks[i].sequence_number = i;
        if (sendto(sockfd, &chunks[i], sizeof(chunks[i]), 0, (struct sockaddr *)&server_addr, addr_len) == -1)
        {
        ...
        while (1)
        {
            ...
            int recv_size = recvfrom(sockfd, &ack, sizeof(ack), 0, (struct sockaddr *)&server_addr, &addr_len);
    ...
    if (recvfrom(sockfd, &total_chunks, sizeof(total_chunks), 0, (struct sockaddr *)&server_addr, &addr_len) == -1)
    {
    ...
    {
        Chunk chunk;
        int recv_size = recvfrom(sockfd, &chunk, sizeof(chunk), 0, (struct sockaddr *)&server_addr, &addr_len);
        if (recv_size == -1)
        {
