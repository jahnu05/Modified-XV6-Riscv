...
void procdump(void)
{
  ...
  printf("\nPID\tSP\tDP\tState\trtime\twtime\tnrun\tstime\tctime");
  printf("\n");
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    // Calculate the Recent Behaviour Index (RBI)
    int val;
        int rbi = 25;
        if (p->rtime + p->stime != 0){
          val = (int)(((3*p->rtime)-(p->stime)-(p->wtime)/ (p->rtime + p->stime+p->wtime+1)) * DEFAULT_PRIORITY);
    ...
    printf("%d\t%d\t%d\t%s\t%d\t%d\t%d\t%d\t%d", p->pid, p->static_priority, dp, state, p->rtime, end_time - p->ctime - p->rtime, p->no_of_times_scheduled,p->stime,p->ctime);
    printf("\n");
  }
}
